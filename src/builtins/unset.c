...
/*   Updated: 2023/05/14 18:59:17 by sizquier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_unset_builtin2(char *var, char ***envp) // funcion que recibe dos parametros, la vble var que deseo eliminar dentro del puntero a un arreglo de cadena de caracteres. tengo que iterar en envp para ver 
{
	...
	{
		if ((ft_strncmp((*envp)[i], var2, ft_strlen(var2)) != 0) && \ //comparo env(i) con var2 en toda la longittud de var, si son distintos
				(ft_strncmp((*envp)[i], var, ft_strlen(var) + 1) != 0)) //comparo env(i) con var en toda su longitud más el caracter null, si son ddistinos
